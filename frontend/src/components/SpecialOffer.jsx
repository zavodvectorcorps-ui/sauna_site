    ...
    try {
      await fetch(`${BACKEND_URL}/api/contact`, {
        ...
        body: JSON.stringify({
          ...
          message: `Specjalna oferta. Długość sauny: ${formData.size}. ${formData.message}`,
          type: 'special_offer',
        }),
      });
      setSubmitted(true);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {gifts.map((gift, i) => (
            <motion.div
              ...
              className="bg-white border border-black/5 p-6 hover:shadow-md transition-shadow group"
            >
              <div className="flex items-start gap-4 mb-4">
                <div className="w-12 h-12 bg-[#C6A87C]/10 flex items-center justify-center flex-shrink-0 group-hover:bg-[#C6A87C]/20 transition-colors">
                  <gift.icon size={24} className="text-[#C6A87C]" />
                </div>
