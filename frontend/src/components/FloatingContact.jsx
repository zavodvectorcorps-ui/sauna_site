...
export const FloatingContact = () => {
  ...
  const waSettings = getSetting('whatsapp_settings');
  const waPhone = (waSettings?.phone_number || phoneClean).replace(/[^0-9]/g, '');
  const waMessage = encodeURIComponent(waSettings?.default_message_pl || 'Dzień dobry! Chciałbym zapytać o saunę.');
    ...
    <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-40 flex flex-col items-end gap-3" data-testid="floating-contact">
      ...
      {waEnabled && (
        <motion.a
          ...
          className="group relative w-14 h-14 bg-[#25D366] flex items-center justify-center shadow-xl hover:bg-[#1fba57] transition-colors rounded-[14px]"
